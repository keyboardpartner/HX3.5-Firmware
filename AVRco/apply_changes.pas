...
// enable/disable warnings for this unit
  ...
  procedure AC_ExecEditChanges;   // Ge�nderte Bedienelemente als Message senden und f�r FPGA sammeln
  ...
  // Wrapper-Forwards f�r Proc-Tabelle:
...
// Diese Tabelle enth�lt alle Indexe auf Routinen-Adressen in ac_proclist,
// die bei �nderung eines edit-Wertes aufgerufen werden m�ssen.
...
ac_proc_idx: Array[0..c_last_edit_param] of byte = (
    ...
    38,  // #1091, Swell Loudness Bass
    38,  // #1092, Swell Midrange Response
    38,  // #1093, Swell Midrange Shelving TonePot Fac
    38,  // #1094, Swell Final Response
    38,  // #1095, Swell Loudness Treble
    ...
    38,  // #1122, B�hm Module Ext Rotary Volume Left
    38,  // #1123, B�hm Module Ext Rotary Volume Right
...
// Konstanten f�r ac_proclist_flags-Index
const
...
var
  ...
  ac_proc_eventsource: Byte; // f�r jeweils aufgerufene Proc g�ltig
...
// Event-Sender: Sorgt daf�r, dass �nderungen nicht an den Absender zur�ckgehen
...
begin
  ...
  case event_source of
    // c_to_fpga_event_source: nur ans FPGA, interne �nderung
    ...
    c_editor_event_source:
      ...
      case ConnectMode of
        t_connect_osc_wifi:
          NB_SendBinaryVal(param, new_val); // OSC braucht das Event zur�ck
          mdelay(5);
          |
      endcase;
      |
...
{$ENDIF}
      ...
      |
    c_midi_event_source:
      // �ber MIDI-CCs verursachte Events
      case ConnectMode of
        t_connect_editor_midi:
          MIDI_SendSysExParam(param, Integer(new_val));
          |
...
// #############################################################################
// #############################################################################
// #####         Interface f�r Prozeduraufruf �ber Tabelle                ######
// #############################################################################
// #############################################################################
...
// Parameter�nderungen in edit_table rufen eine zugeh�rige Routine
...
procedure AC_RouteDirect; // nur f�r Modul ben�tigt
begin
...
procedure AC_HandleReverbKnob;
// �bersetzung in Buttons bereits in AC_MomentaryControls erledigt
begin
...
begin
  for edit_idx:= 128 to 131 do
    AC_sendmsg(edit_idx, c_control_event_source);     // zus�tzlich senden
  endfor;
  AC_SendPercValues;
end;
...
// GM-Programm senden und Namen anfordern, kommt sp�ter �ber SysEx vom DSP
begin
  // Programm setzen
...
{$ENDIF}
  ...
  if edit_LogicalTab_TubeAmpBypass then
    ...
    if Bit(edit_ConfBits, 2) then // Volume Correction bit gesetzt?
      ...
      // nur geringe Korrektur f�r Leslie-Sim
...
const
  // Scale * int_fac < 256, sonst Word-�berlauf!
...
begin
  ...
  if swell_changed or ToneChanged then
    ...
    if edit_EnablePedalAudio then
      ...
      // Pedal-Signal f�r separaten Ausgang auf Extension Board und Postmix:
      swell_pedal:= mulDivByte(swell_pedal, edit_PedalVolume, 128);
      if Bit(edit_ConfBits, 6) then
        // Swell disable f�r separaten Ausgang auf Extension Board und Postmix
    ...
    SendByteToFPGA(swell_final_response, 87); // #1094, 0..63, 5 Bit für Response
    SendByteToFPGA(swell_midrange_response, 88); // #1092, 0..255, 8 Bit für Midrange Response
...
{$IFNDEF MODULE}
  ...
  AC_sendmsg(c_OrganModel, c_preset_event_source);  //  f�r TouchOSC
  AC_sendmsg(c_GatingKnob, c_preset_event_source);  //  f�r TouchOSC
...
{$IFNDEF MODULE}
  ...
  AC_sendmsg(c_SpeakerModel, c_preset_event_source);  //  f�r TouchOSC
...
begin
  ...
  FillBlock(@edit_LogicalTab_IncDecBtns, 16, 0);  // alle IncDec-Buttons l�schen
...
// Aktivierte IncDec-Momentary-Buttons wieder zur�cksetzen
begin
  ac_mb_inc:= false;
  ac_mb_dec:= false;
  ac_mb_v1:= false;
...
{$ENDIF}
  ...
  edit_PercKnob_flag:= 0; // wird sonst evt. zur�ck�bersetzt!
...
procedure AC_MutualControls;
// Wird regelm��ig aus main_tasks-Zeitscheibe aufgerufen
...
begin
  ...
  // alle SingleDBset-Buttons l�schen, nur aktiven setzen
  // edit_LogicalTab_SingleDBtoggle wird in swi_get_TabLED_bits behandelt
  if any_change then
    FillBlock(@edit_SingleDBtoggle, 4, 0);   // alle SingleDBset-Buttons l�schen
    edit_LogicalTab_SingleDBdestBtns[SingleDBsetSelect + 1]:= 255;
  endif;

  resend_osc_colors:= false;

  // Organ-/Rotary-Setup, �nderungen nur �ber Menu oder OSC/MIDI,
  ...
  if edit_GatingKnob_flag > 0 then
    ...
    // alle Tabs l�schen = B3, Flags setzen auf c_board_event_source:
    // edit_LogicalTab_H100_Mode etc. l�schen, Sende-Flags setzen
    // #1152 bis #1155
...
{$IFNDEF MODULE}
    AC_sendmsg(c_GatingKnob, edit_GatingKnob_flag);  //  f�r TouchOSC
{$ENDIF}
  ...
  // Percussion Modes �ber Men� oder #1262 eingestellt?
  if edit_PercKnob_flag > 0 then // �ber Men� ge�ndert
    NB_PercKnobToTabs;
  ...
  // Reverb �ber Men� oder #1263 eingestellt?
  if edit_ReverbKnob_flag > 0 then // �ber Men� ge�ndert
  ...
  1:
    if edit_LogicalTab_4V1_flag > 0 then
      ...
      // Logik: Wechselseitige Ausl�sung oder beide gedr�ckt
      edit_LogicalTab_4V1:= true;
      edit_LogicalTab_4V2:= ac_mb_v2; // nach 75 Ticks gel�scht
      ac_mb_v1:= true;
    ...
    if edit_LogicalTab_4V2_flag > 0 then
      ...
      edit_LogicalTab_4V1:= ac_mb_v1; // nach 75 Ticks gel�scht
      ac_mb_v2:= true;
...
// Wird regelm��ig aus main_tasks-Zeitscheibe aufgerufen
// Ge�nderte Bedienelemente (*_flag > 0) als Message senden und �ber
...
begin
  ...
  // alle Proc-Flags auf "nicht ausf�hren"
  ...
  for idx_w:= 0 to c_last_edit_param do
    ...
    // �nderungen f�r FPGA und DSP, Prozeduraufruf �ber Tabelle
  ...
  for proc_idx:= 0 to c_proc_max - 1 do // ohne "nil"
    if ac_proclist_flags[proc_idx] then
      ...
      run_proc; // aus Tabelle entnommene Routine ausf�hren
    endif;
  endfor;
