...
// Gibt Anzahl der vorhandenen Dateien zur�ck
...
function SD_GetDir(const file_mask: string[5]; const list_files: boolean): Byte;
// Gibt in "number_of_Files" Anzahl der passenden Dateien zur�ck
...
{$ENDIF}
    ...
    F16_FileReset(sd_file); // Datei zum Lesen �ffnen
    DF_unprotect;
    ConfErr:= false;
...
{$ENDIF} // ALLINONE
      ...
      if (block_count > (c_FPGA_lastblock + 1)) or (block_count > word(sd_size)) then
        // maximale L�nge einer Datei �berschritten (SD defekt)?
        ConfErr:= true;
        my_err:= df_writeFailed;
{$IFDEF DEBUG_SD}
        WriteSerError;
...
begin
  ...
  if upd_cores then
    ...
    if (SD_FlashBinFile(c_scan, s_scan_name) > df_noErr) then
      incl(ErrFlags, c_err_upd);  // ScanCore immer ben�tigt!
      is_ok:= false;
    endif;

    if (SD_LoadAndFlashStandardCCsets > df_noErr) then
      incl(ErrFlags, c_err_upd);  // Standard CC Sets immer ben�tigt!
      is_ok:= false;
    endif;

    if (SD_LoadAndFlashTaperings > df_noErr) then
      incl(ErrFlags, c_err_upd);  // Taperings immer ben�tigt!
      is_ok:= false;
    endif;


    if (SD_LoadAndFlashWavesets > df_noErr) then
      incl(ErrFlags, c_err_upd);  // Wavesets immer ben�tigt!
      is_ok:= false;
    endif;
...
// Relative 4k-Blocknummer, Offset hier auf Daten-Bereich!
  ...
  if upd_firmware then
    ...
    // 128 KByte Firmware f�r Bootloader
    if SD_FlashBinFile(c_firmware, s_firmware_name) = df_noErr then  // 992, 128 KByte
      writeln(serout, '/ Firmware copied to DF');
      if overwrite_eeprom then // steht noch im Buffer, nur ins EEPROM �bertragen, wenn TRUE
...
{$IFNDEF MODULE}
      DT_ResetMenuEnables;
{$ENDIF}
      // DFtoEEPROM(c_eeprom_base) wird beim Reboot ausgef�hrt!
