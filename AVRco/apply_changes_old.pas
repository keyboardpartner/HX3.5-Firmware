  t_action = (
    ...
    ta_perc_param,
    ta_organ,
    ta_gating,  // Percussion, EG/H100 Mode, SaveDest- und Men�umfang
    ta_vib, ta_phr_prog,
...
c_edit_actions: Array[0..511] of t_action = (
  ...
  ta_pots, // #1091 B�hm Module Reverb Volume
  ta_pots, // #1092 B�hm Module Efx Volume
  ta_swell, // #1093 B�hm Module Swell Volume
  ta_pots, // #1094 B�hm Module Front Volume
  ta_pots, // #1095 B�hm Module Rear Volume
  ...
  ta_pots, // #1122 B�hm Module Ext Rotary Volume Left
  ta_pots, // #1123 B�hm Module Ext Rotary Volume Right
...
// #############################################################################
// ###                   Tabs auswerten und �nderungen an FPGA               ###
// #############################################################################
Unit apply_changes;
...
function AC_CollectAndSendEventMessages: Boolean;  // Ge�nderte Bedienelemente als Message senden und f�r FPGA sammeln
procedure AC_CollectedActionsToFPGA; // gesammelte �nderungen anwenden
...
c_edit_procs: Array[0..511] of procedure = (
  ...
  @AC_SendVolumes, // #1091 B�hm Module Reverb Volume
  @AC_SendVolumes, // #1092 B�hm Module Efx Volume
  nil, // #1093 B�hm Module Swell Volume
  @AC_SendVolumes, // #1094 B�hm Module Front Volume
  @AC_SendVolumes, // #1095 B�hm Module Rear Volume
  ...
  @AC_SendVolumes, // #1122 B�hm Module Ext Rotary Volume Left
  @AC_SendVolumes, // #1123 B�hm Module Ext Rotary Volume Right
...
begin
  ...
  FH_PercussionParamsToFPGA; // Perc-Bits k�nnten sich ge�ndert haben
end;
...
begin
  ...
  MIDI_SendNRPN($357F, m); // Kanal und Freigabe f�r SAM5504
  AC_SendVolumes;
  ToneChanged:=true;
{$IFDEF DEBUG_AC}
...
procedure AC_SetDetent;
begin
// Mittelpositions-Default ge�ndert, nach zugeh�rigem ADC suchen
  for i:= 0 to 87 do
    n:= ADC_remaps[i];
...
{$IFDEF ALLINONE}
  if Bit(flags, c_sendmidicc) then  // als MIDI-CC senden, �ber Tabelle
...
begin
  ...
  if (edit_ADCconfig >= 2) then
    ...
    if UpperIsLive then
      ...
      ADC_ResetTimersUpper;    // ADC-Kan�le unempfindlich machen (abgelaufen!)
      ADC_ChangeStateAll(false);
      LoadUpperVoice(edit_UpperVoice);
    endif;
  else // Expander, keine Analogeing�nge
...
begin
  ...
  if (edit_ADCconfig >= 2) then
    ...
    if LowerIsLive then
      ...
      ADC_ResetTimersLower; // ADC-Kan�le unempfindlich machen (abgelaufen!)
      ADC_ChangeStateAll(false);
...
begin
  ...
  if (edit_ADCconfig >= 2) then // hat Analogeing�nge
    if is_live then
      ADC_SetChangedPedal; // ADC-Kan�le invalidieren
    else
      LoadPedalVoice(edit_PedalVoice);
      ADC_ResetTimersPedal;    // ADC-Kan�le unempfindlich machen (abgelaufen!)
      ADC_ChangeStateAll(false);
    endif;
  else
...
// #############################################################################
// Action-Dispatcher
// Action-Tabellen entscheiden, was bei Parameter-�nderungen tu tun ist
// #############################################################################
...
4: Sinus 2% THD
5: S�gezahn gefiltert f�r Strings oder Cheesy
6: Sinus 12% k3 f�r Conn
...
begin
  ...
  // 0..2 = B3, 3 = H100, 4 = TOS/LSI Sine,
  // 5 = TOS/LSI Square, 6 = SNG Conn, 7 = Cheesy
  for i:= 5 to 15 do  // F�r Kontakt-Einstellung
  ...
  AC_sendmsg_OR_flag(0266, edit_GenVibMode, CurrentSendFlags);  //  f�r TouchOSC
...
// Generator-Grundeinstellung
// Tapering, Wave, Vibrato, Mixturen, Fu�lagen-Offsets
// 0..2 = B3, 3 = H100, 4 = TOS/LSI Sine,
// 5 = TOS/LSI Square, 6 = SNG Conn, 7 = Cheesy
...
begin
  if edit_OrganSetup < 5 then
    ...
    if edit_OrganSetup = 0 then
      if edit_GenVibMode > 2 then
        edit_GenVibMode:= 0; // Falls H100 oder h�her
      endif;
    else
  ...
  AC_SetGenVibMode;
  AC_sendmsg_OR_flag(0267, edit_GatingMode, CurrentSendFlags);  //  f�r TouchOSC
...
// f�r Preset-�nderungen
begin
  if Bit(CurrentSendFlags, c_sendserial) then
    // DB-�nderungen sofort als Block senden
...
begin
  // edit_LogicalTab_H100_Mode etc. l�schen
...
begin
  ...
  AC_sendmsg_OR_flag(0265, edit_OrganSetup, CurrentSendFlags);  //  f�r TouchOSC
  AC_sendmsg_OR_flag(0267, edit_GatingMode, CurrentSendFlags);  //  f�r TouchOSC umleiten
  for i:= 0 to 3 do  // #1152 bis #1155
     edit_LogicalTabs_KeyingModes_flag[i]:= 0;
  endfor;
...
// Button-LEDs werden erst nach Ablauf von ActivityTimer zur�ckgesetzt.
// Dadurch ist es m�glich, gleichzeitige Bet�tigung mehrerer Buttons
// zu erkennen.
// Hier: Beide Transpose-Buttons zugleich setzen Transponierwert auf 0 zur�ck.
begin
  if edit_LogicalTab_TP_up_flag > 0 then   // ON oder OFF!
    edit_LogicalTab_TP_up_flag:= 0;
    ac_mb_transpose_up:= true;             // ON bis Timer zuschl�gt
    inc(edit_KeyTranspose);
    edit_KeyTranspose_flag:= CurrentSendFlags;
    setsystimer(ActivityTimer, 75);
    if ac_mb_transpose_down then           // beide gedr�ckt?
      edit_KeyTranspose:= 0;
    endif;
  endif;
  ...
  if edit_LogicalTab_TP_down_flag > 0 then  // ON oder OFF!
    ...
    ac_mb_transpose_down:= true;            // ON bis Timer zuschl�gt
    dec(edit_KeyTranspose);
    edit_KeyTranspose_flag:= CurrentSendFlags;
    setsystimer(ActivityTimer, 75);
    if ac_mb_transpose_up then              // beide gedr�ckt?
      edit_KeyTranspose:= 0;
    endif;
  endif;
...
// Aktivierte Momentary-Buttons wieder zur�cksetzen
begin
...
// und Presets (w�rden nach AC_CollectChanges nicht mehr ausgef�hrt)
// Sp�ter AC_CollectChanges zum Zur�cksetzen der alten Werte aufrufen!
var
...
begin
  ...
  ac_preset_changed:= false;  // wird f�r GM-Voicenamen-Unterdr�ckung gebraucht
...
{$IFDEF ALLINONE}
  ...
  or (edit_CommonPreset_flag <> 0) then
    ...
    // Preset ge�ndert, PRE-LOAD in BlockArray
{$IFDEF DEBUG_AC}
...
{$ENDIF}
  ...
  if PresetLoadRequest and IsSysTimerZero(PresetLoadTimer) then
    PresetLoadRequest:= false;
    ac_preset_changed:= true;  // wird f�r GM-Voicenamen-Unterdr�ckung gebraucht
    if (edit_CommonPreset = 0) then
      ...
      SWI_ForceSwitchReload;     // Schalter-Eing�nge neu einlesen
    ...
    MenuIndex_Requested:= c_MenuCommonPreset; // zur�ck zum Hauptmen�
    ...
    if ConnectMode = t_connect_osc_serial then
      MIDI_RequestAllGMnames; // werden f�r Anzeige gebraucht
    endif;
  ...
  if (edit_UpperVoice <> edit_UpperVoice_old) or (edit_UpperVoice_flag <> 0) then
    ...
    send_block_upper; // DB-�nderungen sofort als Block senden
    if (not ac_preset_changed) then
      MenuIndex_Splash:= c_MenuCommonPreset + 1; // neues Men� anfordern
    endif;
  ...
  if (edit_LowerVoice <> edit_LowerVoice_old) or (edit_LowerVoice_flag <> 0) then
    ...
    send_block_lower; // DB-�nderungen sofort als Block senden
    if (not ac_preset_changed) then
      MenuIndex_Splash:= c_MenuCommonPreset + 2; // neues Men� anfordern
    endif;
  ...
  if (edit_PedalVoice <> edit_PedalVoice_old) or (edit_PedalVoice_flag <> 0) then
    ...
    send_block_pedal; // DB-�nderungen sofort als Block senden
    if (not ac_preset_changed) then
      MenuIndex_Splash:= c_MenuCommonPreset + 3; // neues Men� anfordern
    endif;
  ...
  // OrganSetup, �nderungen nur �ber Menu oder OSC/MIDI,
  // NICHT �ber Preset. Belegt einige Parameter f�r jeweiliges Modell.
  if edit_OrganSetup_flag > 0 then
    reset_gatingmode_tabs; // alle l�schen = edit_GatingMode 0 (B3)
    AC_sendmsg_OR_flag(0265, edit_OrganSetup, CurrentSendFlags);  //  f�r TouchOSC
  ...
  // Gating-Mode, 4 Radio Buttons mit gegenseitiger Ausl�sung und OFF
  // Men�-Item oder Knob (RFU) - falls �nderungen �ber Men�
  
  if edit_GatingMode_flag > 0 then
    // Knob ge�ndert. Zugeh�rige Tabs neu setzen und senden
    reset_gatingmode_tabs; // alle l�schen = edit_GatingMode 0 (B3)
    case edit_GatingMode of
      1:
    ...
    resend_osc_colors:= true;
    NB_ValidateExtendedParams;  // Legt g�ltige Men�s und Restore-Freigaben an
  endif;
  ...
  if (not any_gating_tab) and (any_gating_tab_flag > 0) then
    // irgendeine ON-Stellung auf ALL OFF ge�ndert
    reset_gatingmode_tabs; // alle l�schen
    edit_GatingMode:= 0;
    edit_OrganSetup:= 0;
    send_gatingmode_tabs;  // l�scht auch Tab-Flags
    resend_osc_colors:= true;
  endif;
  ...
  if DisablePercussion <> any_change then
    // erzwingt Senden von Upper und Perc �ber ta_perc_param
  ...
  // muss vorab erfolgen, damit Parameter-�nderungen in Schleife erfolgen
  ...
  // Percussion Modes �ber Men� eingestellt?
  ...
  if edit_MenuPercMode_flag > 0 then // �ber Men� ge�ndert
  ...
  // zur�ck�bersetzen, falls �nderung �ber Tabs
...
{$IFDEF ALLINONE}
  if edit_VibKnobMode = 1 then
    ...
    // Logik: Wechselseitige Ausl�sung, nochmaliger Druck schaltet beide ein
    if (edit_LogicalTab_V1_flag > 0) then
      if not edit_LogicalTab_V1 then
        // war vorher ON, nochmals gedr�ckt
  ...
  if edit_VibKnob_flag > 0 then
    NB_VibknobToVCbits;  // Falls �nderung �ber Panel
  endif;
...
{$ENDIF}
  ...
  if edit_SyncPHRtoLeslie and (edit_LogicalTab_LeslieFast_flag > 0) then
    ...
    AC_sendmsg_OR_flag(0150, byte(edit_LogicalTab_PHR_Fast), CurrentSendFlags);  //  f�r TouchOSC
  endif;
...
//    edit_TG_WaveSet_flag:= 0;   // unn�tige �nderung vermeiden
//  endif;
...
// Gegen�ber Parser-Input ge�nderte Bedienelemente als Message senden
// �ber Parser gesetzte Werte sollten nicht unn�tig wieder ausgesendet werden.
// Parser setzt in die Tabelle parsed_table_X die gleichen Werte wie in edit_table_X.
// Vergleich mit diesen Tabellen verhindert, das �nderungen �ber Parser gleich
// wieder als Event-Message gesendet werden.
// Ausnahme: Im Touchpad-CC-Set immer senden (�ber inverse CC-Tabelle)
var
  send_flags: Byte;
...
begin
  // alle �nderungen senden (neuer Wert <> alter Wert)
  // �nderungen nur im Touchpad-Set senden oder wenn mit Editor verbunden
  any_change:= false;
  ...
  for ac_edit_idx:= 0 to 511 do
    ...
    if Bit(send_flags, c_sendfpga) then
      // �nderungen f�r FPGA sammeln
      last_action:= action;
      action:= c_edit_actions[ac_edit_idx];
      if last_action = action then
        // Rest �berspringen, Event wurde bereits gesammelt
        continue;
      endif;
...
{$IFDEF ALLINONE}
  // wenn sich im Upper/Lower/Pedal-Men� Drawbars ge�ndert haben
  if any_change then
...
// Routinen f�r Switch-Auswertung und
// �nderungen in der edit-Tabelle anhand bereits gesetzter Flags auswerten
// wird nur angesprungen, wenn sich in
// AC_CollectAndSendEventMessages etwas ge�ndert hat!
var
  idx, temp_idx: Byte;
{$IFDEF ALLINONE}
...
begin
  ...
  // Abbruchbedingung, falls sich CommonPreset zwischendurch ge�ndert hat
{$IFDEF ALLINONE}
...
{$ENDIF}
        ...
        ta_gmu_v0..ta_gmp_v1:
          ...
          // Namen anfordern, wird �ber SysEx in GM_VoiceNames[] gesetzt
...
{$ENDIF}
          |
        ta_taper_tg:
          if IRQ_EncoderTouched and in_main_menu then  // kann l�nger dauern
...
procedure AC_SendSwell;
// SCHWELLPEDAL-Steuerung, MASTER-Volume wg. MasterSwell f�r B�hm
// wird in MainTasks st�ndig aufgerufen
...
{$IFNDEF ALLINONE}
    my_master_vol, my_front_vol, my_rear_vol, my_efx_vol, my_rev_vol: Byte;  // nur f�r Realorgan
{$ENDIF}
begin
{$IFDEF ALLINONE}
...
{$ENDIF}
  ...
  if SwellPedalChanged or ToneChanged then
    ...
    case edit_PreampSwellType of
      0:
        // Hammond Mode, ausgepr�gtes, aber flaches Maximum bei 200 Hz,
        // ab 250 Hz mit 3-4 db/Okt fallend, �ber 4 kHz st�rker
        // TONE-Pot, Minimal Swell und Swell Trim Cap werden ber�cksichtigt
       ...
       // my_swell_1khz:= MulDivByte(my_swell_1khz, my_swell_tone_inv, 127);
        ...
        // 4kHz-Anteil wird mit steigendem TONE-Wert gr��er
        // in AO28-Sim ist dieser Pegel um -12 dB abgesenkt
      ...
      1:  // Conn, B�hm etc. Sinus
        ...
        // Frequenz 120,3 Hz * (Wert + 1) + 128 f�r 4k Enhanced
...
// Nur RealOrgan!
  ...
  // m wird overall Volume, regelt alle Ausg�nge
...
begin
  ...
  if my_val < 64 then
    ...
    // oberhalb Mittelstellung, n >= 64, l�uft von 64 bis 127
...
{$IFDEF ALLINONE}
  ...
  if edit_LogicalTab_TubeAmpBypass then
    ...
    // edit_LeslieInpLvl korrigieren, um �bersteuerungen zu vermeiden
  ...
  // SendVolumeByteLogToFPGA(m, 45);  // 45 = Master Vol ALT auf 0..255, �ber Log-Table
...
{$IFDEF ALLINONE}
  ...
  // bei �nderungen wird EQ in SAM-FW neu initialisiert
...
begin
  ...
  if edit_LeslieBalance >= 64 then
    ...
    // Hornpegel konstant, unabh�ngig von Balance
    ...
    // Rotorpegel konstant, unabh�ngig von Balance
    SendByteToFPGA(edit_LeslieInits[6], 182);
  endif;
  
  // Horn-Throb konstant, unabh�ngig von Balance
  ...
  // Rotor-Throb konstant, unabh�ngig von Balance
