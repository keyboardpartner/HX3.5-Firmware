...
// kopiert 4K-Bl�cke von first_block bis einschl. last_block nach dest_block:
...
// Liefert TRUE wenn Transfer erfolgreich (Pr�fsumme OK)
...
// vorhandenen 4K-Block aus BlockBuffer8 in DataFlash �bertragen
// Liefert TRUE wenn Store erfolgreich (kein Speicherfehler)
function DF_Store4kBlock(const block_4k, block_len: Word): boolean;

// Pr�fsumme Firmware direkt aus DF
// df_first_block: erster zu ber�cksichtigender 4K-Block
// df_last_block: letzter zu ber�cksichtigender 4K-Block
...
var
{$PData}
  F_DFCS[@PortB, b_DFCS]: bit;    // CS f�r AT25DF021
...
procedure DF_sr;
// Sende/empfange ein Byte �ber SPI an DF (F_DFCS muss entspr. gesetzt sein)
begin
  asm;
;    cli ; Disable interrupts
  ...
  DF_waitReg:
    ...
    in _ACCA, SPDR
    sts  dataflash.DF_receive, _ACCA  ;Lesewert zur�ck ins Datenbyte
...
procedure DF_busy;
// Warte, bis DF nicht mehr besch�ftigt ist
begin
  F_DFCS:= low;
  DF_send:=$05; // Read Status
  DF_sr;
...
function DF_eraseblock_4k(const block_4k: Word): boolean;
// L�sche 4-KByte-Block bzw. 64-KByte-Sektor im DF
...
function DF_erase: boolean;
// DataFlash l�schen, warten bis Ende
// liefert TRUE wenn erfolgreich
begin
  DF_wen;
  F_DFCS:= low;
...
// es k�nnen max. 256 Bytes auf einmal geschrieben werden
var my_idx: word;
  df_adr: LongInt;
begin
  my_idx:= 0;
...
begin
  ...
  DF_send:=DF_long0; // Adr Bits 7..0
  DF_sr;
  DF_send:=0; // dummy f�r $0B read mode
  DF_sr;
...
procedure DF_FWupdateFromFlashAndReboot;
//Firmware aus DataFlash holen, per Bootloader in AVR �bertragen und Reboot
begin
  LEDactivity:=low;
{$IFNDEF MODULE}
  if LCDpresent then
...
{$ENDIF} // ALLINONE
  ...
  // Flash-Programmierung im Bootblock anspringen. L�dt ROM aus SPI-Flash
  // und startet anschlie�end neu. Da update.ini inzwischen umbenannt
  // ist, wird dieser Vorgang nur einmal ausgef�hrt. Bei
