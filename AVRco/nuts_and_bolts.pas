...
uses const_def, var_def, port_def, edit_changes, fpga_if, eeprom_def, dataflash;
  ...
  // sendet einen bin�r kodierten Header ohne L�nge an serielle Schnittstelle:
...
//  procedure NB_pulse(const p_len: byte); // neg. 10�s/dig Impuls f�r Messungen mit LA
...
procedure NB_VibknobToVCbits;
// setzt Vibrato-Knopfstellung in logische Tab-Stellung f�r LEDs um
begin
  ...
  // nur Knob-Stellung ber�cksichtigen:
  FillBlock(@edit_LogicalTab_VibBtns_flag, 6, 0);
  MenuRefresh:= true;
end;

procedure NB_VCbitsToVibknob;
// setzt logische Tab-Stellung f�r LEDs in Vibrato-Knopfstellung um
begin
  // Sonderfall Button Vibrato mit gegenseitiger Ausl�sung, V/C einzeln
  // nur Vib-Tabs
  if (edit_VibKnobMode = 3) then
    edit_VibKnob:= 0;
    for i:= 1 to 5 do
...
begin
  if TWIstat(i2c_addr) then
    ...
    udelay(10); // f�r FatarScan76 n�tig!
...
procedure NB_CreateInverseCCarrays;
// CC-Arrays f�r MIDI IN/OUT anlegen
// ch enth�lt auch Flags: Bit 4 = Inverted, Bit 5 = Scaled
var my_idx: Integer;
    cc, ch: Byte;
begin
  UseSustainSostMask:= $80;
  FillBlock(@CCarray, 1024, 255); // -1 = ung�ltiger Parameter
...
begin
  ...
  // BlockBuffer8 enth�lt jetzt Block aus 4 x 768 Parametern
...
procedure NB_init_edittable;
// Defaults aus Flash f�r Preset-Initialisierung
var idx_w: Word;
begin
...
procedure NB_CreateInverseMenuArr;
// Das RestoreArray entscheidet, ob ein Wert aus CommonPresets �bernommen wird
...
begin
  ...
  // edit_RestoreCommonPresetMask Bits 1 bis 6 im Men�
  for i:= 0 to c_MenuLen - 1 do      // im Men�, falls save_dest < EEPROM
...
procedure NB_ValidateExtendedParams;
// Ung�ltige Werte anhand nicht freigegebener Bedienelemente setzen
var
   my_param: Integer;
   my_idx, my_mask: byte;
begin
  for my_idx:= 0 to c_MenuLen do
    ...
    if (not HasExtendedLicence) and (not Bit(my_mask, 6)) then
      // Eintrag ung�ltig, Default-Wert 0 in edit_table
...
{
procedure NB_pulse(const p_len: byte); // neg. Impuls f�r Messungen mit LA
begin
  excl(PortD, 3);
  udelay(p_len);
  incl(PortD, 3);
...
// Routinen zum bin�r kodierten Senden (schneller als Parser in Klartext)

function NB_SendBinaryHeader(const cmd: byte; const param_num: Integer):byte;
// sendet einen bin�r kodierten Header ohne L�nge an BLE bzw. serielle Schnittstelle
// ESC CMD ADRL ADRH
// liefert CRC zur�ck
var
  my_temp_crc : byte;
...
{$IFNDEF MODULE}
  ...
  LED_timer1000;
{$ENDIF}
  MenuIndex_Requested:= MenuIndex; // zur�ck zum letzen Men�
end;
...
procedure NB_SendBinaryBlock(const param_num: Integer; const len: Byte);
// mit Event-CMD #5: Erwarte Best�tigung nach Bearbeitung
...
begin
  ...
  repeat
    ...
    if ConnectMode = t_connect_osc_wifi then
      ...
      done:= true; // nur einmal ausf�hren wenn kein ACK erwartet
    endif;
    inc(retry);
...
// ACHTUNG: AutoInc funktioniert beim PCA9532 aus unbekannten Gr�nden
// NICHT bei PSC- und PWM-Registern, deshalb hier "zu Fu�":
