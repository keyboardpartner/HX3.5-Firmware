...
// MenuIndex um menu_delta erh�hen/erniedrigen, Wrap around ausf�hren
...
// Holt Menupanel-Button-Status und liefert TRUE wenn Up/Down/Enter gedr�ckt
...
function DT_GetMenuMax(const my_menu_index: byte): byte;
// liefert Maximalwert des Men�eintrags �ber c_edit_max-Tabelle
var
  arr_idx: Integer;
  max_val: byte;
begin
...
var
  ...
  max_val: byte;
begin
  // mit Beschleuningung f�r Drawbar/Poti-Werte
  asm;
    CLI;
  endasm;
  ...
  // Integrierer f�r Encoder-Absolutwert
...
function DT_MenuEntryValid(const menu_idx: byte): t_menuvalid;
// Men�-Eintrag vorhanden? Ist nur g�ltig, wenn KEIN phys.
// Bedienelement vorhanden ist und Lizenz den Eintrag zul�sst.
// wird bei jedem Men�wechsel aufgerufen (Up/Down/Enter)
// type t_menuvalid = (t_menu_hidden, t_menu_invalid, t_menu_valid);
var
...
// Start und Ende des aktuellen Men�-Bereichs ermitteln
begin
...
// MenuIndex um menu_delta erh�hen/erniedrigen, ggf. Wrap around ausf�hren
...
begin
  ...
  repeat
    // wiederholen, bis g�ltiges Men� gefunden
    search_index:= search_index + byte(delta);
    // if Bit(edit_ConfBits, 5) then  // Wrap Menus?
      if (search_index > 200) or (search_index < first_menu) then // �berlauf Byte
    ...
    // Ausgehend vom letzen Men� ein g�ltiges in Richtung Delta suchen;
...
procedure DT_SetUpDownArrows;
// Up/Down-Arrows f�r derzeitige Richtung anhand des
// n�chsten und vorherigen Eintrags setzen
// Wei�e Up/Down-Arrows, wenn Submenu vorhanden
begin
  LCDset_LR_Arrows(ValueChangeMode); // filled wenn ValueChangeMode

  LCDxy_M(LCD_m1, 15, 0);
  LCDOut_M(#6);   // Updown, Farbe abh�ngig von ValueChangeMode
  LCDxy_M(LCD_m1, 15, 1);
  if IsInMainMenu then
    LCDOut_M(#32);
  else
    LCDOut_M(#5); // inv. "s" wenn im Submen�
  endif;
...
procedure DisplayHeader(const my_str: String[15]);
// Display-�berschrift aus Array, Rest l�schen und in zweite Zeile wechseln
begin
  LCDxy_M(LCD_m1, 0, 0);
