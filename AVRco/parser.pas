...
//
// #############################################################################
// ###             Parser f�r Befehle von serieller Schnittstelle           ###
// #############################################################################
unit parser;
...
// auch f�r Men�system benutzt. Liefert TRUE wenn Parameter bekannt ist
...
const
// nur noch allgemeine und Konfigurations-Parameter �ber Mnemonics!
  ...
  CmdStrArr      : array[0..cmdAnzahl] of string[3] = (
    ...
    'INI',// 8300 INI-Datei ausf�hren
    'SCI',// 8500 Scan Core Info, 0=ID, 1=Revision
    'RCB',// 8600 4K-Block �berspezielles Protokoll empfangen
...
begin
  ...
  case my_param of
{$IFDEF MODULE}
    900..905:  // Dummy f�r neuen Editor mit Spartan-7 auf B�hm Modul
               my_result:= 0;
               |
{$ELSE}
...
{$ENDIF}
    2100..2163 : // Leslie Inits
               ...
               |
    2200..2327 : // 8 Vibrato Parameter-Tabellen f�r OEM Modul
...
begin
  ...
  case my_param of
    ...
    8204      :// DSP-Signatur/Version, aktuell geladen
              ...
              |
    8500:     // ScanCore Info
              FI_GetScanCoreInfo;  // Meldung �ber Scancore ausgeben
    ...
    8529:      // HashadowMem f�r FPGA und ScanCore; liefert bei alten Versionen Fehler -1
              WriteByteSer(0);
              |
...
// #############################################################################
// #####           STANDARD-EVENT: Parameter-Wert ge�ndert                 #####
// #############################################################################
...
begin
  ...
  // in edit_array, nicht in EEPROM, m�glichst schnell erledigen
  case idx_i of
{$IFNDEF MODULE}
    0000..0011:
...
{$ENDIF}
    0121, 0128..0191:      // Tabs?
      // wichtig, sonst wird "not XXX" bei Werten <> 255 falsch ausgef�hrt!
...
begin
  ...
  case my_param of
    250:
      EEunlocked:= my_bool;  // f�r Binary-Parser
      |
{$IFDEF MODULE}
    900..909:  // Dummy f�r neuen Editor mit Spartan-7 auf B�hm Modul
      |
{$ELSE}
...
//      |
    ...
    1609:
      ...
      |
    1610:// Start OSCconnectedBySerial Mode, HX3 sendet Parameter-�nderungen, bin�r
      MIDI_RequestAllGMnames;
      FlushBuffer(RxBuffer);
      ConnectMode:= t_connect_osc_wifi;  // Abschalten mit 9902=0
      DisplayHeader('OSC Connect...');
      mdelay(100);
      NB_SendBinaryVal(1610, 127); // Connect Button ON (dummy f�r Re-Connect)
      mdelay(1000);    // Zeit f�r IP-Meldung von WiFi-Interface
      NB_SendBinaryVal(1610, 127); // Connect Button ON
      NB_SendBinaryVal(1649, 64); // LED Blink
      writeln(serout, '/page/1=0'); // f�r alte Versionen
      ...
      mdelay(100);    // Zeit f�r IP-Meldung von WiFi-Interface
      ...
      writeln(serout, '/page/1=127'); // f�r alte Versionen
      mdelay(100);    // Zeit f�r IP-Meldung von WiFi-Interface
    ...
    1620..1627:
      ...
      |
    1628..1639:  // INCs/DECs f�r Custom MIDI, wurde bereits in WIFI-IF erledigt!
      if my_bool then
    ...
    1650: // #1650 TouchOSC Page Select, Einstellung vorbelegen
      ...
      case value of
        ...
        writeln(serout); // sync Text
        writeln(serout, '/label_wait=" "'); // "Please Wait..." l�schen
        |
        1,2: // B3/Basic Pages
        edit_OrganModel:= 0;
        ...
        edit_OrganModel_flag:= event_source;
        |
        5,6: // EG Mode Pages, keine �nderung wenn bereits EG Mode
...
begin
  ...
  case my_param of
    0..239     :// FPGA-Schreibregister Wort, auch Default-Auto-Increment
               TimeSlot:= 15;// z�hlt wieder rauf bis 0, SPI-Updates �ber AVR verhindern
    ...
    4000..4767:// Custom-CC-Tabellen beschreiben, tempor�r
...
{$ENDIF}
               |
    4768..4799:// Custom-NRPN-Tabellen beschreiben, tempor�r
               my_param:= my_param - 4768;
...
{$ENDIF}
    ...
    8200, 8201:      // UPD, FileLoad from SD to SPI Flash, alle Standard-Dateien versuchen
               ...
               if my_bool and SD_ForceCheck then
                 SD_LoadAndFlashAllBinCores(my_index_10 = 0, true);// anschlie�end FW-Flash und Reset!
                 if ConfErr then
    ...
    8203:      // UPD 3, Scan Driver only
               if my_bool and SD_ForceCheck then
                 if SD_FlashBinFile(c_scan, s_scan_name) > df_noErr then
                   serprompt(c_err_conf, my_param, -1);      // ScanCore immer ben�tigt!
                   return;
                 endif;
                 START_InitAll;
               endif;
...
{$ENDIF}
    ...
    8211:      // UPD 11, Wavesets
               ...
               |
    8299:      // UPD 99, DF Erase, Chip ganz l�schen
               if my_bool then
                 DF_unprotect;
    ...
    8500:      // Firmware Update from DF auf Controller, falls �ber SysEx oder seriell geladen
...
{$ENDIF}
    ...
    8510:      // Firmware Update von DF auf Controller, falls �ber SysEx oder seriell geladen
               // Wie 8500, aber OHNE �berpr�fung der Checksumme!
{$IFNDEF MODULE}
               DT_ResetMenuEnables;
{$ENDIF}
               DF_FWupdateFromFlashAndReboot;
    ...
    8511:      // EEPROM Update von DF auf Controller
               // Wie 8501, aber OHNE �berpr�fung der Checksumme!
...
{$ENDIF}
    ...
    8600:      // Empfange 4KByte-Block
               if ValueInt < 32 then
                 ValueInt:= 4096;     // Kompatibilit�t mit alten HX3.5-Editor
               endif;
               serOut(#6); // ACK
    ...
    8601:      // Soeben empfangenen 4KByte-Block in DF speichern, absolute Blocknummer
               // Liefert OK wenn L�schen/Speichern erfolgreich
{$IFNDEF MODULE}
...
{$ENDIF}
               ...
               |
{$IFNDEF MODULE}
    8605:      // 4KByte-Block als tempor�res Preset laden
               if my_bool then
...
{$ENDIF}
    ...
    8900..8915:
               ...
               0, 2: // Scan Core und FIR, LongWords
                 ...
                 for my_word:= 0 to word(ValueInt) - 1 do // L�nge in LongWords!
               ...
               1, 3: // Tapering und Keymap, Bytes
                 FI_AutoIncSetup(my_index_100); // for Write
                 for my_word:= 0 to word(ValueInt) - 1 do // L�nge in Bytes!
                   FPGAsendByte:= serInp;
                   SendFPGA8;
                 endfor;
                 FI_AutoIncReset(my_index_100);
                 ...
                 FI_AutoIncSetup(my_index_100); // for Write
                 for my_word:= 0 to word(ValueInt) - 1 do // L�nge in Words!
...
{$IFNDEF MODULE}
    9100, 9101:// Display Message Line n
               if ParamAlpha then
                 ...
                 LCDclreol_M(LCD_m1);
                 setSysTimer(ActivityTimer, 1000); // zur�ck nach 2 Sekunden
...
{$ENDIF}
    9900, 9901: // ConnectMode nach benutzter Schnittstelle einstellen:
               if CmdSentBySerial then  // HX3 sendet Parameter-�nderungen, bin�r
                 ConnectMode:= t_connect_editor_serial;
               endif;
               if CmdSentByMIDI then // HX3 sendet Parameter-�nderungen, Sysex
...
begin
  ...
  ParamAlpha:= false;
  myBool:= false;
  while SerInpStr[ParsePtr] = ' ' do // Leerzeichen �berspringen
    Inc(ParsePtr);
  endwhile;
...
begin
  ...
  verbose:= (pos('!', SerInpStr) > 0);// OK erw�nscht?
...
// CRC = einfache Modulo-Pr�fsumme �ber alle Bytes von ESC bis DATAn
...
// mit
// LEN = Anzahl der gew�nschten Datenbytes, 0 => 256
// CRC = einfache Modulo-Pr�fsumme �ber alle empfangenen Bytes von ESC bis LEN
...
// mit
// LEN = Anzahl der folgenden oder gew�nschten Datenbytes, 0 => 256
// CRC = einfache Modulo-Pr�fsumme �ber alle Bytes von ESC bis DATAn
...
begin
  ...
  if BinaryValid then
    if (BinaryCmd = 1) then //    or (BinaryCmd = 3)
      ...
      // auf Datensatz warten, L�nge steht fest. Buffer f�llen
      // Mit Cmd=1 �nderungen nicht erneut senden
      // Mit Cmd=3 wird der ge�nderte Parameter sp�ter als
      // ge�ndert erkannt und zur�ckgesendet
      ...
      if BinaryValid then
        // Datensatz vollst�ndig, CRC stimmt. Befehl best�tigen.
        // Nur an FPGA, nicht zur�cksenden, kommt von Editor oder OSC Wifi
        for i:= 0 to BinaryLen-1 do
  ...
  if not BinaryValid then
    while serStat do // ung�ltigen Rest lesen
      serInp;
    endwhile;
  endif;
  SerInpStr:= '';
    ...
    case my_char of
      ...
      #13:  // CR
        if (length(SerInpStr) > 0) and SerInpStr[1] <> '/' then
          ...
          PA_HandleCmdString;  // Befehl vollst�ndig, also interpretieren
        ...
        SerInpStr:= '';
        |
      #27:  // ESC --> Bin�rmodus starten, niemals auf EEPROM loslassen!
        EEunlocked:= false;
        CmdSentByMidi:= false;
        CmdSentBySerial:= true;
        PA_HandleBinary;
...
var script_found: Boolean; my_byte: Byte;
  bytes_read: word;
begin
  MenuIndex_Requested:= MenuIndex; // Anzeige zur�cksetzen wenn beendet
...
{$ENDIF} // ALLINONE
    ...
    // Falls Firmware-Update erw�nscht, wird nach Flashen des AVR jetzt
    // ein Reset ausgef�hrt.
