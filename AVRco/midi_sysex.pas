...
{$ENDIF}
  ...
  // Standard-Antwort auf Parameter-�nderungen und Fehlerabfrage:
...
// MIDI-CC-Sets:
//  'NI B4 d3c  ' , // 0, interpretiert, teilw. �ber Custom 'ccset0.dat'
//  'Hammond XK ' , // 1, interpretiert, teilw. �ber Custom 'ccset1.dat'
//  'Hammond SK ' , // 2, interpretiert, teilw. �ber Custom 'ccset2.dat'
//  'Versatile  ' , // 3, interpretiert, teilw. �ber Custom 'ccset3.dat'
...
{$I MIDI_Custom.pas}   // f�r CC-Sets 4..10
{$I MIDI_ni.pas}
{$I MIDI_sempra.pas}
...
begin
   ...
   until my_adr >= my_count; // L�nge erreicht?
...
begin
  ...
  if edit_MIDI_Option <> 0 then    // ggf tempor�r auf Ausgang schalten
    edit_MIDI_Option:= 0;          // Senden hardwarem��ig freigeben
...
// NN nn = 14Bit-Parameter-Anzahl (1 = 1 Wert, 0 unzul�ssig)
// wir senden:
...
// #############################################################################
// Eigene SysEx behandeln. SysEx-ID von Keyswerk/B�hm oder Roland
...
// #############################################################################
// Rudiment�rer Interpreter f�r Roland VK77, MIDI Kanal 1,2,3
//
...
{$ENDIF}
    ...
    elsif (lo(SysExCmd_sempra) = $33) then  // Empfangen: ID f�r uns
...
// #############################################################################
      ...
      if (sysex_id = $07) then  // Empfangen: Parameter setzen
        ...
        repeat
          ...
          // Sysex kann nur �ber Editor kommen, kein Feedback
...
{$ENDIF}
        until false;
        // ACK sp�ter senden, wenn �nderungen abgeschlossen
...
{$ENDIF}
        PA_HandleCmdString;  // wie Befehl �ber Com-Schnittstelle
        SerinpStr:='';
        // ACK sp�ter senden, wenn �nderungen abgeschlossen
        MIDI_SendSysEx_status;
...
// #############################################################################
// Empfangen: einzelnen Parameter 1000...7999 abfragen, Antwort bin�r ($05)
...
// CS = 7-Bit-Pr�fsumme �ber alle Parameter- und DatenBytes PP pp VV vv VV vv...
// Werte VV vv werden N-mal wiederholt, jeweils eine Parameternummer h�her
// #############################################################################
...
{$ENDIF}
        ...
        until (my_idx > 43);
        block_page_cs:= SysExArray[my_idx];   // Pr�fsumme �ber alle Datenbytes
...
// VV vv = 14-Bit-L�nge, max. 4096 Bytes pro Seite
// #############################################################################

      elsif (sysex_id = $21) then
        my_idx:= 6;
        if MIDI_GetSysEx_int(my_idx, my_param) then  // Blocknummer
          if MIDI_GetSysEx_int(my_idx, my_val) then  // L�nge
{$IFNDEF MODULE}
...
// VV vv = 14Bit-L�nge Bytes (00VVVVVV Vvvvvvvv wird zu 0VVVVVV 0vvvvvvv)
// wir senden:
// F0 00 20 04 33 1B PP pp VV vv <7+1 data> <7+1 data> ... CC F7
// mit
// CC = 7-Bit-Pr�fsumme �ber alle Daten- und MSBytes
// #############################################################################
...
// VV vv = 14Bit-Page-L�nge (00VVVVVV Vvvvvvvv wird zu 0VVVVVV 0vvvvvvv)
// wir senden:
...
// #############################################################################
// ErrFlags abfragen und l�schen ($01)
...
begin
  if mv = $F0 then  // SysEx-Beginn, 8 Bit zul�ssig wg. SPI
    SysExCount:= 0;
    SysExActive:= true;
    // innerhalb 50ms muss n�chstes SyEx-Byte eintreffen
  ...
  if (mv = $F7) then  // vollst�ndig, korrektes Ende
    MIDI_DispatchSysEx;
    SysExCount:= 0;
    SysExActive:= false;
  endif;
...
// MIDI IN �berpr�fen und ggf. an MIDI-Interpreter weiterleiten
begin
...
{$ENDIF}
      ...
      if not mch_is_valid then // Channel f�r uns?
        continue;
      endif;

      mch_idx:= mch - edit_MIDI_Channel;

      // auf Bank Selects reagieren, auch f�r Eder TopShop
      if (mcmd = $B0) and (mch_idx <= 3) then
        case mp of
           0: // Bank Select MSB von Genos f�r Eder TopShop
            BankSelectGenosValids[mch_idx]:= (mv = 63) and (BankSelectLSBs[mch_idx] = 120);
            BankSelectMSBs[mch_idx]:= mv;
            MIDI_SetGenosActive(mch_idx);
            |
          32: // Bank Select LSB von Genos f�r Eder TopShop
      ...
      // Program Changes allgemein, kann ggf. in CC-Sets noch ge�ndert werden
...
{$ENDIF}
      ...
      // Spezielle Controller au�erhalb MIDI-CC-Set Interpreter
      if mcmd = $B0 then
        midi_received:= FPGAreceiveLong and $00FFFFFF;  // 00, cmd, cc, val
{$IFNDEF MODULE}
        if edit_ShowCC then
        ...
        // F�r Touchpad und Custom CC, abschalten mit val >
        if (mp = 124) then
          ...
          if valueInRange(mv, 0, 10) then
            ...
            if edit_MIDI_CC_Set = 8 then     // MIDI-CCs immer zur�cksenden
...
{$ENDIF}
  ...
  // wieder gesetzt. Wenn zu lange nichts kommt, ist SysEx abgest�rzt.
