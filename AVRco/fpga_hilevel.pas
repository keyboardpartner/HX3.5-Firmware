...
// LC#    Breite   L�nge Bytes  LC Core
// 0        24        8192      PicoBlaze       (Datei/DF-Blocks)
// 1         8        1024      Taper-RAM (Datei/DF in 32 Bit, nur unterste 8 �bertragen)
...
// 1024 HighpassFilter 16 Bit breit an FPGA DDS48 �bertragen
  procedure FH_NoteHighpassFilterToFPGA;

// 95 Tuning-Werte 16 Bit breit an FPGA DDS96 �bertragen
  procedure FH_TuningValsToFPGA;

// 8 Bit breit an FPGA TAPER �bertragen
...
var
  ...
  // erhalten �bersetzte Werte aus edit_UpperRoutingWords
  ...
  bb_attack_arr: array[0..15] of byte;   // Zeitkorrekturen f�r EG Decay Mode
  bb_decay_arr : array[0..15] of byte;  // Zeitkorrekturen f�r EG Decay Mode
  bb_sustain_arr: array[0..15] of byte;   // Zeitkorrekturen f�r EG Decay Mode
  bb_release_arr: array[0..15] of byte;   // Zeitkorrekturen f�r EG Decay Mode
...
procedure FH_SendReverbTabs;
// Vorbereitete Parameter-Tabelle UPPER f�r HX3-Engine an FPGA
{
...
begin
  ...
  if ReverbKnob_old <> edit_ReverbKnob then
    // eigener Event f�r Reverb Level w�re sicher eleganter...
...
// bei denen zus�tzlich der Sustain-Pegel vom normalen
// Fu�lagen-Drawbar statt von ADSR-Sustain-Poti �bernommen werden soll.
var
...
{$ENDIF}
  ...
  for i:= 0 to 15 do       // 16 Werte log. an LC senden
    ...
    if is_eg_adsr and bit(bb_ena_env_percmode_bits, i) then  // soll auf Drawbar-Level?
      ...
      if mute_db then
        // Drawbar-Pegelabsenkung bei Percussion gew�nscht, nur Hammond
...
// produziert 15 abfallende/ansteigende ADSR-Werte f�r Saiten-Simulation
// In "bb_ena_env_adsrmode_bits" sind jene Bits auf '1',
// bei denen ein BUSBAR auf ADSR geschaltet ist.
// In "bb_ena_env_percmode_bits" sind jene Bits auf '1',
// bei denen zus�tzlich der Sustain-Pegel vom normalen
// Fu�lagen-Drawbar statt von ADSR-Sustain-Poti �bernommen werden soll.
// drawbars_to_lc muss ausgef�hrt sein!
var
...
begin
  ...
  for i:= 0 to 15 do
    ...
    // EG Percussion Mode aktiviert? Sustain-Wert �ndern
...
{$ENDIF}
    ...
    // HARMONIC DECAY, modifizierte Zeiten f�r obere Teilt�ne
...
begin
  // Decay-Value f�r Harp-Sustain-Fu�lage 4' = BB 2
  bb_attack_arr[3]:= 0;
...
// liefert zugeh�rige Fu�lagen-Bits nach Drawbar-Z�hlweise, oberes Byte
var
...
// *****************************ALLINONE****************************************
  ...
  if HasExtendedLicence then
    if (edit_GatingKnob = 1) then
      ...
      if edit_LogicalTab_H100_HarpSustain then
        // H100 HarpSustain: 4' zus�tzlich auf ADSR, voller Pegel, Dry Channel
...
procedure FH_UpperRoutingToFPGA;
// edit-Routing-Bits in beteiligte Fu�lagen umrechnen und an FPGA senden
// bei Sempra direkte �nderungen �ber MIDI
// ena_cont_perc_bits     = 32, mit Bit 15 = PERC_BYPASS
// ENA_CONT_BITS     = 40, mechanische Kontakte f�r jede Fu�lage
// ENA_ENV_DB_BITS   = 41, elektronische Kontakte f�r jede Fu�lage
// ENA_ENV_FULL_BITS = 42, elektronische Kontakte f�r jede Fu�lage
// ENV_TO_DRY_BITS   = 43, ADSR-Fu�lagen auf Dry-Kanal
...
procedure FH_PercussionParamsToFPGA;
// f�r Advanced Routing und Hammond-Percussion
...
// *****************************ALLINONE****************************************
  ...
  // Bei H100 und EG Mode ist zus�tzliche trockene Percussion m�glich
  ...
  if ena_h100_perc then  // H100, Perc-Level verringern
    if edit_LogicalTab_H100_2ndVoice then
      ...
      // Gesetzte Bits in bb_ena_cont_perc_bits z�hlen und
      // Percussion Volume um so mehr verringern, sonst �bersteuerung
      m:= 0;
      my_med_lvl:= 0;
      for i:= 0 to 15 do
...
// ******************************MODULE*****************************************
  ...
  // Simple Version f�r B�hm, kein H100-Mode
  m:= 0;
  my_med_lvl:= 0;
  for i:= 0 to 15 do
    if bit(bb_ena_cont_perc_bits, i) then
...
// *****************************************************************************
  ...
  // Routing-Bits werden durch erfolgte �nderungen an FPGA gesendet
end;
...
begin
  ...
  for i:= 0 to 15 do
    ...
    FPGAsendWord:= c_TimeLogTable[m];
    // modifizierte Zeiten f�r obere Teilt�ne
...
procedure FH_PedalDrawbarsToFPGA;
// Vorbereitete Parameter-Tabelle BASS f�r HX3-Engine an FPGA
...
{$ENDIF}
  ...
  SendByteToFPGA(edit_KeyTranspose, 13);  // positive Werte verschieben T�ne nach UNTEN!
  ...
  if ForceSplitRequest and edit_LogicalTab_SplitOn then
    // Scan/Split erstmal OFF, damit �nderungen beim Wiedereinschalten �bernommen werden
{$IFDEF DEBUG_FH}
...
{$ENDIF}
  ...
  SendByteToFPGA(m, 9);          // Klick-L�nge und Noise-Frequenz
  // Transpose, positive Werte verschieben T�ne nach UNTEN!
...
// F�r jede Oktave des Generators muss eine eigene, 1024 Worte lange Wellenform
// geladen werden. H�here Oktaven d�rfen wg. Nyquist-Grenze keine Oberwellen
// jenseits 20 kHz enthalten.
// Wg. Nicht-H�rbarkeit der Oberwellen werden Oktaven 6 bis 8 (9) grunds�tzlich
...
procedure FH_TubeCurveToFPGA(const tube_set_a, tube_set_b: byte);
// 256 Step- und 256 Slope-Werte f�r interpolierenden TubeAmp HX4.0
var
  stepval, slopeval: Integer;
begin
  // Werte f�r LC vorbereiten aus Slope-Tabelle
  stepval:= 0;
  for i:= 0 to 31 do
    slopeval:= c_tubeampslopes[tube_set_a, i];
    SlopeArray[i]:= slopeval;
    StepArray[i]:= stepval;
    stepval:= stepval + slopeval;  // n�chste Stufe um slope h�her
  endfor;
  ...
  // negative Werte, r�ckw�rts
  stepval:= 0;
  for i:= 255 downto 224 do
    slopeval:= c_tubeampslopes[tube_set_b, 255-i];
    SlopeArray[i]:= slopeval;
    StepArray[i]:= stepval;
    stepval:= stepval - slopeval;  // n�chste Stufe um slope kleiner
  endfor;
...
// f�r jede Oktave. Lediglich oberste Hammond-Oktave ist etws gespreizt, deshalb extra.
var
...
// *****************************ALLINONE****************************************
        4: // linear
           for m:= 0 to 63 do
             ...
             SendFPGA32;  // 3 Bytes unbenutzt, aber f�r Tapering Editor gebraucht!
           endfor;
         |
...
{$ENDIF}
  ...
  if (edit_DB16_FoldbMode and 2) = 2 then  // muted, fr�her TWG Config0
...
{$ENDIF}
  ...
  // Defaultwerte f�r FPGA ggf modifizieren
  ...
  // Wenn kein FB Invert, FB Level zur�cknehmen
{$IFNDEF MODULE}
...
{$ENDIF}
  ...
  for i:= 0 to 14 do   // 15 ansteigende Verz�gerungszeiten berechnen
...
// aufgeteilt auf drei einzelne Bytes
  ...
  if edit_LogicalTab_LeslieRun then
    // Motoren laufen, gew�nschte Geschwindigkeiten ann�hern
    ...
    // Rampe f�r Auslauf, stoppt auf bestimmter Position
