...
{$IDATA}
{
  ...
  procedure ADC_ChangeStateAll(const my_state: Boolean); // ADCs freigeben, erzwingt sp�tere Aktualisierung
...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// ADC-Remap �ber Tabelle: Index ist ADC-Kanal, Eintrag ist edit_LogicalTabsTable-Index
...
var
  ADC_changed : Array[0..127] of boolean; // Flags: Analog-Wert hat sich ge�ndert
  // 100..108: Secondary DB Set 1
  // 112..120: Secondary DB Set 2

// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// ADC-Remap �ber Tabelle: Index ADC-Kanal, Ausgang edit_Table-Index
// Werte aus Index-Tabelle:
// 0..79 in edit_table_0
// 80..239 in edit_table_1
// 240..254 Sonderbehandlung
...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  // 100..108: Secondary DB Set 1, gleiche Werte wie 0..9    (f�r DBX9)
  // 112..120: Secondary DB Set 2, gleiche Werte wie 12..21  (f�r DBX9)
...
// Schreibt in Tabelle ADC_Values, wenn Eingang sich st�rker als
// Hysterese �ndert. Dieser Eingang wird dann �ber Timer auf "aktiv" gesetzt
// und laufend in ADC_Values aktualisiert.
// Wert in ADC_Values speichern und ADC_changed-Flag setzen, sofern ge�ndert
// liefert Remap-Wert zur�ck, der ohnehin gebraucht wird
// 18 �s f�r je jeden freigeschalteten Eingang im Leerlauf (nicht bedient)
// 20 �s, wenn Wert sich gerade �ndert
// benutzt n
var
  ...
  my_active : boolean;
begin
  // Einschwingzeit nutzen, Zeit bis zur Wandlung liegt insgesamt bei ca. 8 �s
  ...
  if sr_pulse then
    ADCSR:= $93;  // Clear ADIF
    ADCSR:= $C3;  // Start, ca. 8 �s nach SR-Takt bzw. MPX-Umschaltung
    // Konvertierzeit nutzen
    adc_val_old_i:= ADC_Values[adc_chan]; // vorheriger ADC-Wert
    repeat
    until Bit(ADCSR, 4);         // bis ADIF-Bit gesetzt
    ADCSR:= $83;  // Stopp
    // n�chster HC164-Ausgang, Einschwingzeit nutzen
    incl(PortD, 5); // HC164 CLK
    nop; nop;
    excl(PortD, 5); // HC164 CLK
  else
    // Einschwingzeit verl�ngern f�r Poti in DB-Expander, evt. 50k
  ...
  // neuer Wert um grobe Hysterese gr��er oder kleiner als alter Wert?
  ...
  // falls ge�ndert, neuen Wert und Flag in ADC-Array setzen
  if my_active and (adc_raw_i <> adc_val_old_i) then
    ADC_Values[adc_chan]:= adc_raw_i; // letzter Wert f�r n�chsten Vergleich
...
// Werte in edit_table speichern und �nderungs-Flags setzen
// 740 �s mit 24 Eing�ngen im Leerlauf
// PA4: INHIBIT MPX 0
// PA5: INHIBIT MPX 1
// PA6: INHIBIT MPX 2
// PA7: ANALOG BUS
...
{$IFNDEF MODULE}
  if ADCtestMode or (edit_ADCconfig >= 2) then
    ...
    // vor der Wandlung ggf. einmalig DB9-MPX-Satz ausw�hlen
...
// Werte in edit_table speichern und �nderungs-Flags setzen
// 19 �s f�r je jeden freigeschalteten Eingang im Leerlauf (nicht bedient)
...
{$IFNDEF MODULE}
    ...
    if (edit_ADCconfig = 3) then
      ...
      if valueInRange(my_remap, 0, 11) then          // Upper DBs?
        if is_2nd_dbset_remap <> UpperSecondaryActive then
          return;                                   // nicht aktiv, �berspringen
        endif;
  ...
  // normale Zuordnung �ber Remap, kein Rotary-DB
  if edit_ADCconfig = 2 then       // alte DB9-MPX mit 4053-Umschaltern
    if UpperSecondaryActive_DB9_MPX and valueInRange(adc_remap, 0, 8) then
      return; // nicht eintragen, wird sp�ter erledigt
    endif;
  ...
  // Orgel mit nur einem DB-Satz
    ...
    if SingleDBsetSelect = 1 then
      ...
      if valueInRange(adc_remap, 16, 24) then   // Lower DBs?
        // �nderungen auf Lower-Eing�ngen verwerfen
        return;
      endif;
    endif;
  endif;
...
procedure ADC_ChangesToEdit;
// �nderungen von ADC-Werten in edit_table setzen
var
...
{$IFDEF EDER_FS76}
  if ExternalScanActive and (edit_ADCconfig = 5) then
    // Nur f�r Orgeln mit FatarScan76
    // Anzahl gedr�ckter Tasten vom Scan-Interface holen.
...
{$ENDIF}
  ...
  // Wenn bei altem DB9-MPX zweiter Drawbar-Satz gew�hlt ist,
  // wurden die Drawbar-ADC-Werte auf 100..108 und 112..120 abgelegt
  if UpperSecondaryActive_DB9_MPX then
    for adc_chan:= 100 to 108 do
      // Verteilt ADC-Werte von zweitem DB9-MPX, wenn ge�ndert.
      adc_remap:= ADC_remaps[adc_chan];
      if adc_remap = c_map_none then
        continue;
      endif;
...
// Schweller-Eingang PA2 wird immer gewandelt und bei �nderung Change-Flag gesetzt
var
  my_adc_new : integer;
begin
...
// Analoge Eing�nge und Timer auf "changed" setzen
...
begin
  ...
  // 100..108: Secondary DB Set 1, gleiche Werte wie 0..9   (f�r DB9-MPX, alt)
  for i:= 0 to 8 do
  ...
  // 112..118: Secondary DB Set 2, gleiche Werte wie 12..21 (f�r DB9-MPX, alt)
  for i:= 12 to 20 do
    ADC_remaps[i + 100]:= ADC_remaps[i];
  endfor;
